...
from misc import ( _get_latest_repo_pkg_meta, _get_pkg_meta, _fmt_chlog,
                   _find_containers, _release_date, _regroup_repo_packages,
                   _search )
...
def _deref(plat, symb):
    if symb == "latest":
        ...
        pointer = Pointer.objects.filter(name = symb, target__platform__name = plat)
        if pointer.count():
...
class GraphForm(forms.ModelForm):
    ...
    def clean(self):
        ...
        if repos and image:
            raise ValidationError('Please choose only an image or set of repos, not both')
        if not repos and not image:
            raise ValidationError('Please choose either an image or a set of repos')
        ...
        if packages:
            ...
            if image:
                ...
                if len(not_in_image):
                    raise ValidationError('Package names %s are not in image %s' % (",".join(not_in_image), image))
            else:
                not_in_repos = set(copy(pkgs))
                for repo in repos:
                    not_in_repos = not_in_repos - set(repo.packages)
                if len(not_in_repos):
                    raise ValidationError('Package names %s are not in selected repos' % ",".join(not_in_repos))
...
class GraphAdmin(admin.ModelAdmin):
    list_display = ( '__unicode__', 'related_container', 'has_pkg_meta', 'packages_report' )
    raw_id_fields = ['repo']
    ...
    def dot_url(self, graph):
        if graph.dot and os.path.exists(os.path.join(settings.MEDIA_ROOT, graph.dot.name)):
            return "<a href='" + os.path.join("/", settings.MEDIA_URL, graph.dot.name) + "'>Download</a>"
        else:
    ...
    def svg_url(self, graph):
        if graph.svg and os.path.exists(os.path.join(settings.MEDIA_ROOT, graph.svg.name)):
            return "<a href='" + os.path.join("/", settings.MEDIA_URL, graph.svg.name) + "'>Download</a> or " + \
                   "<a href='" + reverse('admin:reports_graph_view', args=(graph.id,)) + "'>View</a>"
        elif graph.dot and os.path.exists(os.path.join(settings.MEDIA_ROOT, graph.dot.name)):
            return "Not yet rendered.<a href='" + reverse('admin:reports_graph_view', args=(graph.id,))  + "'>Render and view</a>"
            ...
            if repo.components.count() == 0:
                ...
                for r in repo.components.all():
                    if r.is_live and r.projects.count() == 1:
                        pkgs.update({str(r.platform) + " " + str(r.projects.all()[0]) : r.packages})
                    else:
                ...
                for mtype in PackageMetaType.objects.all():
                    ...
                    if mtype.allow_multiple:
                        ...
                        for choice in mchoices:
                            pkg_data[repo][pkg][mtype.name].update({ choice.name: False })
                    elif mtype.default:
    ...
    def view(self, request, graphid):
        ...
        if not graph.svg or not os.path.exists(os.path.join(settings.MEDIA_ROOT, graph.svg.name)):
        ...
        return TemplateResponse(request, 'graph.html', context = context, current_app=self.admin_site.name)
    ...
    def view_testreport(self, request, graphid, repoid=None):
        ...
        if request.method == 'POST' and request.user.has_perms("reports.add_packagemetatype"):
            repository = request.POST.get('repository')
            pkgs       = request.POST.getlist('package')
            pkg_meta_types = list(PackageMetaType.objects.all())
            # generate list of keys that are present on POST and are not key for another dict
            getlist_keys = set([x.name for x in pkg_meta_types if not x.allow_multiple]) & set(request.POST.keys())
            getlist_items = {}
            ...
            # checkbox part
            checkbox_types = [x for x in PackageMetaType.objects.all() if x.allow_multiple ]
            for ctype in checkbox_types:
                if not ctype.name in graph.pkg_meta[repository][pkg].keys():
                ...
                for cb_choice in cb_choices:
                    for pkg in pkgs:
                        if not ctype.name in graph.pkg_meta[repository][pkg].keys():
                            graph.pkg_meta[repository][pkg][ctype.name] = {}
                        graph.pkg_meta[repository][pkg][ctype.name].update({cb_choice: False})
                # check relevant checkboxes
                for cb_choice in cb_choices:
                    for pkg in request.POST.getlist(cb_choice):
                        graph.pkg_meta[repository][pkg][ctype.name].update({cb_choice: True})
        ...
        # generate pkg_meta by copying from latest repo/graph that has it set and then update
        # against repo packages.
        if graph and not graph.pkg_meta:
            old_meta = _find_previous_pkg_meta(graph)
            if old_meta and not 'live' in container.release:
        ...
        if graph and graph.packages:
            ...
            for repo in graph.pkg_meta.keys():
                for pkg in graph.pkg_meta[repo].keys():
                    if not pkg in graph_pkgs:
                        del graph.pkg_meta[repo][pkg]

        pkg_state = PackageMetaType.objects.filter(name='state')
        ce_fields = PackageMetaType.objects.filter(name='ce_related')
        ...
        context = {
          ...
          'graph' : graph,
          'container' : container,
          'metatypes' : list(PackageMetaType.objects.all().select_related("default").prefetch_related("choices")),
          'latest_image' : img,
        ...
        if "certification" in request.path:
            return TemplateResponse(request, 'certification.html', context = context, current_app=self.admin_site.name)
        else:
            return TemplateResponse(request, 'testreport.html', context = context, current_app=self.admin_site.name)
...
class ABIAdmin(admin.ModelAdmin):
    ...
    def abi_export(self, request, pk):
        ...
        print abi
        return HttpResponse(json.dumps(abi.listing), content_type='application/json')
        
    def abi_list(self, abi):

        return '<a href="' + reverse('admin:reports_abi_export', args=(abi.id,)) + '?format=json">Get JSON</a>'
...
class RepoAdmin(admin.ModelAdmin):
    ...
    def listall(self, request):
        ...
        for p in Platform.objects.all():
            repos = Repo.objects.filter(platform__name = p, containers = None).order_by('-release_date', '-release')
            repos = repos.select_related('platform', 'server').prefetch_related('projects',"note_set")
            if repos.count():
        ...
        return TemplateResponse(request, 'list.html', context = context, current_app=self.admin_site.name)
    ...
    def search(self, request):
        ...
        if request.method == "GET":
            ...
            if form.is_valid():
                ...
                for pointer in form.cleaned_data["Targets"]:
                    ...
                    pkgs, newcaps = _search(querytype, query, container, exact=exact, casei=casei)
                    results[str(container)] = pkgs
                    capidx[str(container)] = newcaps
        else:
            return HttpReponseNotAllowed(['GET', 'POST'])
        ...
        return TemplateResponse(request, 'search.html', context = context, current_app=self.admin_site.name)
    ...
    def diff(self, request, new_repoid, old_repoid):
        ...
        new_repo = Repo.objects.select_related("server", "platform").prefetch_related("projects", "components", "containers").get(pk = new_repoid)
        old_repo = Repo.objects.select_related("server", "platform").prefetch_related("projects", "components", "containers").get(pk = old_repoid)
        ...
        if request.method == 'POST':
            ...
            comment = "Request by %s from repodiff of %s to %s" % (request.user, new_repo, old_repo)
            ...
            return TemplateResponse(request, 'diff_noprogress.html', context = context, current_app=self.admin_site.name)
        ...
        progress_cb("Done")
        return TemplateResponse(request, "diff_content.html", context = context, current_app=self.admin_site.name)
...
class ImageAdmin(admin.ModelAdmin):
    ...
    def listall(self, request):
        ...
        images = Image.objects.select_related("container_repo").prefetch_related("container_repo__platform").all().order_by("-container_repo__release")
        for image in images:
