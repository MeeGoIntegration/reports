...
from serializers import RepoSerializer, RepoServerSerializer, ReleaseSerializer
...
class RepoViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    """
    queryset = Repo.objects.filter(components=None).select_related("server", "platform").prefetch_related("archs").order_by('-release_date', '-release')
...
class ReleaseViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    queryset = Pointer.objects.select_related("target").prefetch_related("target__images", "target__note_set").all().order_by("-target__release")
    serializer_class = ReleaseSerializer
 
def releases(request):

    releases = Pointer.objects.select_related("target").prefetch_related("target__images", "target__note_set").all().order_by("-target__release")
